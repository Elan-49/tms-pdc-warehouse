...
function wireObserve(){
  ...
  const applyManual=()=>{const observed=Number($('#manualObservedTime').value);if(!Number.isFinite(observed)||observed<=0){alert('Isi Total Waktu Pengamatan dengan angka lebih dari 0.');return false}state.manualTime=+observed.toFixed(2);state.start=null;state.end=null;refreshTimes();saveObserveDraft();return true};
  ...
  const restoreObserveVideo=async()=>{
    ...
    try{
      ...
      if($('#observationSessionId'))$('#observationSessionId').textContent=state.observationSessionId?observationSessionLabel(state.observationSessionId,$('#date').value):'Belum ada video';
      refreshTimes();
  ...
  $('#setStart').onclick=()=>{if(state.observationMethod==='manual')return;if(!hasVideo()){alert('Pilih video terlebih dahulu.');return}state.start=+video.currentTime.toFixed(2);state.end=null;state.manualTime=null;$('#manualObservedTime').value='';refreshTimes();saveObserveDraft();video.play().catch(()=>{})};
  $('#setEnd').onclick=()=>{if(state.observationMethod==='manual')return;if(!hasVideo()){alert('Pilih video terlebih dahulu.');return}video.pause();state.end=+video.currentTime.toFixed(2);state.manualTime=null;if(state.start!=null&&state.end<state.start){alert('End harus lebih besar dari Start');state.end=null;return}refreshTimes();saveObserveDraft()};
...
function renderMaster(){
 ...
 function modal(existing={},editIndex=null){if(!ensureWrite())return;
  ...
  const captureMasterDraft=()=>({editIndex,id:existing.id||null,process:$('#mProcess')?.value||'',activity:$('#mActivity')?.value||'',element:$('#mElement')?.value||'',classification:$('#mClass')?.value||'',waste:$('#mWaste')?.value||'-',method:normalizeMasterMethod($('#mMethod')?.value),equipment:$('#mEquipment')?.value||'-',frequency:+$('#mFreq')?.value||0,notes:$('#mNotes')?.value||'',savedAt:Date.now()});
  const saveMasterDraft=()=>scheduleDraft('master',captureMasterDraft);
  ['mProcess','mActivity','mElement','mClass','mWaste','mMethod','mEquipment','mFreq','mNotes'].forEach(id=>{$('#'+id)?.addEventListener('input',saveMasterDraft);$('#'+id)?.addEventListener('change',saveMasterDraft)});
...
function renderTSKK(skipCloud=false){
  ...
  <div class="tskk-meta-helper"><span><b>Source:</b> satu Observation/video. Work Element, Start, End, dan Actual Time tidak diinput ulang. Type ditarik otomatis dari Metode pada Master Data.</span></div>
  ...
  const updateTakt=()=>{const av=+$('#tskkAvailable').value||0,units=+$('#tskkUnits').value||0;selected.availableMinutes=av;selected.requiredUnits=units;selected.taktTime=av>0&&units>0?+(av*60/units).toFixed(2):0;const input=$('#tskkTakt');if(input)input.value=selected.taktTime>0?fmtTimeValue(selected.taktTime):'—';draw();};
...
$$('#nav button[data-view]').forEach(b=>b.onclick=()=>{if(b.dataset.role==='admin'&&!isAdmin()){alert('Menu ini hanya dapat diakses Admin.');return}captureActiveDraft();state.view=b.dataset.view;state.tskkEditor=false;state.tskkDraftId=null;history.replaceState({appView:state.view,tskkEditor:false,tskkId:null},'',location.href);setSidebar(false);render()});
