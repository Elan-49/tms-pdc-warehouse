...
function renderTSKK(skipCloud=false){
  ...
  // The editor template uses displayUnit in its column labels. It must be
  // available before innerHTML is evaluated; otherwise creating a TSKK throws
  // ReferenceError and the Back / Save / Print handlers are never attached.
  const initialCalc=tskkCalc(selected);
  const initialDisplayUnit=tskkTimeUnit(Math.max(initialCalc.cycle,initialCalc.takt,1));
