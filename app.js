...
function renderTSKK(skipCloud=false){
  ...
  const draw=()=>{selected.items=selected.items.map(x=>{const start=Number.isFinite(+x.start)?Math.max(0,+x.start):0;const time=Math.max(0,+x.time||0);const derivedType=tskkTypeFromMaster(x.element||'');const safeType=['manual','auto','walk'].includes(derivedType)?derivedType:(['manual','auto','walk'].includes(x.type)?x.type:'manual');return {...x,type:safeType,start,end:(Number.isFinite(+x.end)?Math.max(start,+x.end):start+time),time}});const cc=tskkCalc(selected);$('#tskkWorkBody').innerHTML=isManualObservation?(selected.items.length?selected.items.map((x,i)=>`<tr><td>${i+1}</td><td><div class="tskk-readonly-element">${esc(x.element||'—')}</div></td><td><div class="tskk-readonly-type ${x.type||'manual'}">${esc(tskkTypeLabel(x.type||'manual'))}</div></td><td>${i===0?`<div class="tskk-readonly-number"><b>${fmt(cc.cycle)} dtk</b></div>`:'<span class="muted">Satu cycle untuk seluruh observation</span>'}</td></tr>`).join(''):'<tr><td colspan="4" class="empty">Observation belum memiliki element context.</td></tr>'):(selected.items.length?selected.items.map((x,i)=>`<tr><td>${i+1}</td><td><div class="tskk-readonly-element">${esc(x.element||'—')}</div></td><td><div class="tskk-readonly-type ${x.type||'manual'}">${esc(tskkTypeLabel(x.type||'manual'))}</div></td><td><div class="tskk-readonly-number">${fmt(x.time)}</div></td><td><div class="tskk-readonly-number">${fmt(x.start)}</div></td><td><div class="tskk-readonly-number">${fmt(x.end)}</div></td><td><input class="tskk-item-note" data-i="${i}" value="${esc(x.note||'')}" ${readOnly?'disabled':''}></td></tr>`).join(''):'<tr><td colspan="7" class="empty">Observation belum memiliki element.</td></tr>');$('#tskkKpis').innerHTML=`${kpi('TAKT TIME',fmt(cc.takt)+' dtk','Target pace')}${kpi('ACTUAL CYCLE TIME',fmt(cc.cycle)+' dtk',cc.gap>=0?'Masih di bawah takt':'Melebihi takt')}${isManualObservation?'':kpi('MANUAL + WALK',fmt(cc.operatorWork)+' dtk',`Load operator ${fmt(cc.operatorLoad)}%`)}${isManualObservation?'':kpi('AUTO / MACHINE',fmt(cc.auto)+' dtk',`Proporsi ${fmt(cc.machineShare)}%`)}`;$('#tskkTimeline').innerHTML=isManualObservation?`<div class="manual-cycle-summary"><div><span>ACTUAL CYCLE</span><b>${fmt(cc.cycle)} dtk</b></div><div><span>TAKT TIME</span><b>${fmt(cc.takt)} dtk</b></div><div><span>VARIANCE</span><b>${fmt(cc.gap)} dtk</b></div><div><span>STATUS</span><b>${esc(cc.status)}</b></div></div>`:`<div class="tskk-chart-scroll">${tskkTimeline(selected)}</div>`;$('#tskkInsight').innerHTML=`<b>Evaluasi:</b> ${tskkStatusBadge(cc)} <span>${cc.walk>0?`Walking ${fmt(cc.walk)} detik.`:'Belum ada aktivitas walking.'}</span>`;$$('.tskk-item-note').forEach(el=>el.oninput=()=>selected.items[+el.dataset.i].note=el.value)};
