...
async function bootCloud(){
  ...
  }catch(err){console.error(err);showToast(err?.code==='AUTH_ACCESS_DENIED'?'Akun belum memiliki akses aktif. Hubungi administrator.':(err?.message||'Backend cloud belum dapat dimuat.'),'error',5000);$('#loginScreen').classList.remove('hidden');$('#appShell').classList.add('hidden');return;}
  render();
}
