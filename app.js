...
function wireObserve(){
  ...
  const applyManual=()=>{const observed=Number($('#manualObservedTime').value);if(!Number.isFinite(observed)||observed<=0){alert('Isi Total Waktu Pengamatan dengan angka lebih dari 0.');return false}state.manualTime=+observed.toFixed(2);state.start=null;state.end=null;refreshTimes();refreshObservationDisplay();return true};
