...
function renderUsers(){
  ...
  (async()=>{
    ...
    const rows=data||[];
    $('#userTable').innerHTML='<div class="table-wrap"><table class="data-table"><thead><tr><th>Nama</th><th>Email</th><th>Role</th><th>Status</th><th>Dibuat</th><th>Aksi</th></tr></thead><tbody>'+rows.map(u=>`<tr data-user="${u.id}"><td>${esc(u.full_name||'—')}</td><td>${esc(u.email)}</td><td><select class="user-role"><option ${u.role==='viewer'?'selected':''}>viewer</option><option ${u.role==='analyst'?'selected':''}>analyst</option><option ${u.role==='admin'?'selected':''}>admin</option></select></td><td><span class="user-status-pill user-status-${esc(u.status)}">${u.status==='approved'?'Approved':u.status==='pending'?'Pending':'Suspended'}</span><select class="user-status user-status-select" aria-label="Status pengguna"><option ${u.status==='pending'?'selected':''}>pending</option><option ${u.status==='approved'?'selected':''}>approved</option><option ${u.status==='suspended'?'selected':''}>suspended</option></select></td><td>${new Date(u.created_at).toLocaleDateString('id-ID')}</td><td><button class="btn primary user-save" data-id="${u.id}">Simpan</button></td></tr>`).join('')+'</tbody></table></div>';
    $$('.user-status-select').forEach(sel=>sel.addEventListener('change',()=>{const pill=sel.parentElement.querySelector('.user-status-pill');const v=sel.value;pill.className='user-status-pill user-status-'+v;pill.textContent=v==='approved'?'Approved':v==='pending'?'Pending':'Suspended';}));
