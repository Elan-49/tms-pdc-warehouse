...
function analysisTable(mode='category'){
  ...
  return `<div class="table-wrap"><table class="data-table validation-analysis-table">
    <thead><tr>
      ...
      <th>N Raw</th><th>N In Limits</th><th>N′</th><th>Mean</th><th>St Dev</th><th>UCL</th><th>LCL</th><th>Outlier</th><th>Status</th>
  ...
  $('#app').innerHTML=`<div class="content page-validation">
    ...
    <div class="card section validation-parameters">
      ...
      <div class="form-grid four validation-parameter-grid">
        <label>N Minimum Observasi Awal (N)<input id="minInitialN" type="number" min="2" step="1" value="${settings.minInitialN}"><small>Gate penelitian sebelum uji kecukupan; bukan pengganti N'</small></label>
    ...
    <div class="grid cols-3 validation-summary">
      ...
      <div class="card"><h3>Uji Keseragaman</h3><p class="muted">Memeriksa keseragaman observasi individual dengan batas ±3 SD. Nilai 3 SD adalah metode tetap dan tidak terkait langsung dengan pilihan confidence untuk N'.</p><div class="formula-box"><b>UCL</b> = Mean + 3 × St Dev<br><b>LCL</b> = Mean − 3 × St Dev<br><small>Implementasi LCL dibatasi minimum 0 karena waktu tidak bernilai negatif.</small></div><p class="muted">Data di luar UCL/LCL ditandai sebagai outlier. Kelompok yang tidak seragam tidak digunakan untuk penetapan Standard Time.</p></div>
      <div class="card"><h3>Uji Kecukupan</h3><p class="muted">Kecukupan dihitung setelah data seragam. Confidence dan precision adalah parameter penelitian; Z diturunkan otomatis dari confidence.</p><div class="formula-box"><b>N'</b> = ⌈[(Z / p) × √(N × Σx² − (Σx)²) / Σx]²⌉<br><small>Keyakinan = ${confidence}% &nbsp;•&nbsp; Z = ${zValue} &nbsp;•&nbsp; s = ${precisionPct.toFixed(1)}% (${(Number(settings.precision)||0.05)})</small></div><p class="muted"><b>N Minimum Observasi Awal = ${settings.minInitialN} observasi.</b> Keputusan sufficient tetap mensyaratkan data seragam dan N Valid ≥ N'.</p></div>
    </div>
...
function renderRating(){
  ...
  $$('#app .rating-delete-pic').forEach(btn=>btn.onclick=async()=>{
    ...
    if(!(await tmwaDialog.confirm(`PIC "${name}" akan dihapus dari Rating Factor.${detail}${historyNote}`,{title:'Hapus PIC dari Rating Factor?',confirmText:'Hapus',danger:true})))return;
    btn.disabled=true;
    try{
